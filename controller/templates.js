APP.templates = (function (){
	...
	function tagList(data){
		...
		for (var j in hash){
			output = output + '<dd><a href="#'+ j +'">' + j + '(' + hash[j] + ')' + '</a></dd>';
		}
	...
	function titleList(notes){
		...
		for (i = 0; i < notes.length; i++){
			if (notes[i].key == 1){
				output = output + '<dd><a href="#' + notes[i].key + '">'+ notes[i].value.Title +'</a></dd>';

			}else{
				output = output + '<dd><a href="#' + notes[i].key + '">'+ notes[i].value.Title +'</a> <button class="deleteA" onclick="APP.notesController.deleteNote('+notes[i].key+');">删除</button><button class="editA" onclick="APP.notesController.editNote('+notes[i].key+');">编辑</button></dd>';
			}
		}
	...
	function contents(note,contents) {
		...
		return '<div id="markdownview"><h3>标题: '+ note.Title + "</h3>" + "<h4><strong>标签: </strong>" + note.Tags.join(",") + "</h4><h4><strong>发表时间: </strong>"+ note.PublishTime+'</h4><br/>'+ contents + '</div';
	}
